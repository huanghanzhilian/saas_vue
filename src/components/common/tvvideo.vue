        ...
        <div class="videolist_info_wrap">
          ...
          <hgroup class="videolist_right">
            <header class="video_detail_header">
              <h4 class="video_title ellipsis">{{item.sourceName}}</h4>
            </header>
            <h5 class="video_detail_title_wrap">
                <p class="video_detail_title ellipsis">{{item.name}}</p>
            </h5>
            <h5 class="video_describe_wrap">
              <p class="video_describe">
                  <!-- <span>{{item.watchAmount}}</span> -->
...
<script>
import { detailTvVideoList } from 'src/service/getData'
...
export default {
  ...
  methods: {
    async initData() {
      //获取数据
      let res = await detailTvVideoList(this.page, this.querySize, this.gameId);
